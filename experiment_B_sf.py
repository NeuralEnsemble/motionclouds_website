...
if False:
    ...
    z_low = mc.envelope_donut(fx, fy, ft, B_sf=0.037, loggabor=False)
    z_high = mc.envelope_donut(fx, fy, ft, B_sf=0.15, loggabor=False)
    
    import pylab, numpy
    pylab.clf()
    fig = pylab.figure(figsize=(12, 12))
    a1 = fig.add_subplot(111)
    a1.contour(numpy.fliplr(z_low[:N_X/2,N_Y/2,N_frame/2:].T), [z_low.max()*.5], colors='red')
    a1.contour(numpy.fliplr(z_high[:N_X/2,N_Y/2,N_frame/2:].T), [z_high.max()*.5], colors='blue')
    a1.set_xlabel('spatial frequency')
    a1.set_ylabel('temporal frequency')
    fig.savefig(mc.figpath + name + '_envelope_overlap.eps')

if False: 
    #create 2 movies in opposite directions
    seed = 12345
    z = color * mc.envelope_donut(fx, fy, ft, V_X=1.)
    mc.visualize(fx, fy, ft, z, name=name_)
    mc.anim_save(mc.rectif(mc.random_cloud(z)), name_ + '-right', display=display, vext=vext)
    mc.anim_save(mc.rectif(mc.random_cloud(z)), name_ + '-left', display=display, vext=vext, mirror=True)
    
    # checking for different frequencies
    for sf_0 in [0.1 , 0.2, 0.3, 0.8]:
        name_ = mc.figpath + name + '-sf_0' + str(sf_0)
        z = color * mc.envelope_donut(fx, fy, ft, sf_0=sf_0)
    ...
    for V_X in [1./4, 1./2 , 1. , 2.0]:
        name_ = mc.figpath + name + '-V_X' + str(V_X)
        z = color * mc.envelope_donut(fx, fy, ft, V_X=V_X)
        mc.visualize(fx, fy, ft, z, name=name_)
        mc.anim_save(mc.rectif(mc.random_cloud(z)), name_, display=display, vext=vext)
    
    for B_sf in [0.0125, 0.025, 0.05]:
        name_ = mc.figpath + name + '-B_sf' + str(sf_0)
        z = color * mc.envelope_donut(fx, fy, ft, B_sf=B_sf)
        mc.visualize(fx, fy, ft, z, name=name_)
        mc.anim_save(mc.rectif(mc.random_cloud(z)), name_, display=display, vext=vext)
    ...
    for seed in [123456, 123457, 123458, 123459]:
        ...
        z = mc.rectif(mc.random_cloud(color * mc.envelope_donut(fx, fy, ft), seed=seed))
        mc.anim_save(z, name_, display=display, vext=vext)

    
    for V_Y in [0.5 , 1.0 , 2.0]:
        name_ = mc.figpath + name + '-V_Y' + str(V_Y)
        z = color*mc.envelope_donut(fx, fy ,ft ,V_Y =V_Y)
        mc.visualize(fx, fy, ft, z, name=name_)
        mc.anim_save(mc.rectif(mc.random_cloud(z)), name_, display=display, vext=vext)
    ...
    # checking for different frequencies
    for sf_0 in [0.1 , 0.2, 0.3, 0.8]:
        for V_X in [0.0, 0.1 , 1.0 , 4.0]:
            name_ = mc.figpath + name + '-sf_0' + str(sf_0)  + '-V_X' + str(V_X)
            z = color * mc.envelope_donut(fx, fy, ft, sf_0 = sf_0)
            mc.visualize(fx, fy, ft, z, name=name_)
            mc.anim_save(mc.rectif(mc.random_cloud(z)), name_, display=display, vext=vext)
